# Void Release Build Script (Windows ARM64)
# Purpose: Build a production-ready release of Void for Windows ARM64

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  Void Release Builder (ARM64)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[5/5] Building Windows ARM64 release..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "========================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "Output location: ..\VSCode-win32-arm64\" -ForegroundColor Cyan
    Write-Host ""

    # Check if output exists
    $outputPath = Resolve-Path "..\VSCode-win32-arm64" -ErrorAction SilentlyContinue
    if ($outputPath) {
        Write-Host "Build contents:" -ForegroundColor Yellow
        Get-ChildItem $outputPath | Select-Object Name, Length | Format-Table
        Write-Host ""
        Write-Host "To run: ..\VSCode-win32-arm64\CoderChat" -ForegroundColor Cyan
    }
} else {
    Write-Host ""
